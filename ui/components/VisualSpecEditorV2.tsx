...
export default function VisualSpecEditorV2({ spec, onSave, onGenerateCode }: VisualSpecEditorV2Props) {
    ...
    // Filter endpoints based on search and selected methods
    ...
    const displayedEndpoints = useMemo(() => {
        ...
        // Default: Sample first 5 endpoints across groups
                ...
                <div className="flex gap-4 items-center">
                    ...
                    <div className="flex border border-gray-300 rounded overflow-hidden">
                        <button
                            ...
                            className={`px-3 py-1 text-sm ${viewMode === 'tree' ? 'bg-orange-50 text-orange-600' : 'bg-white hover:bg-gray-50'}`}
                            title="Tree View"
                        >
                            📁 Tree
                        </button>
                        <button
                            ...
                            className={`px-3 py-1 border-x border-gray-300 text-sm ${viewMode === 'list' ? 'bg-orange-50 text-orange-600' : 'bg-white hover:bg-gray-50'}`}
                            title="List View"
                        >
                            📋 List
                        </button>
                        <button
                            ...
                            className={`px-3 py-1 text-sm ${viewMode === 'cards' ? 'bg-orange-50 text-orange-600' : 'bg-white hover:bg-gray-50'}`}
                            title="Card View"
                        >
                            🎴 Cards
                        </button>
                    ...
                    {!searchQuery && !showAllEndpoints && (
                        <div className="flex flex-wrap gap-2 items-center">
                            ...
                            {Object.entries(filteredGroups).map(([tag, endpoints]) => (
                                ...
                                >
                                    📁 {tag} <span className="text-xs opacity-75">({endpoints.length})</span>
                                </button>
                            ))}
                        </div>
                    )}
                            ...
                            Object.entries(displayedEndpoints).map(([tag, endpoints]) => (
                                ...
                                >
                                    <span className="flex items-center gap-2">
                                        <span className="text-lg">{expandedGroups.has(tag) ? '📂' : '📁'}</span>
                ...
                <div key={tag} className="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <span className="text-2xl">{expandedGroups.has(tag) ? '📂' : '📁'}</span>
                    ...
                    >
                        <div className="p-4">
                            ...
                            <div className="flex items-center gap-3 mb-4 text-xs text-gray-500">
                                {hasAuth && <span className="flex items-center gap-1">🔐 Auth</span>}
                                {paramCount > 0 && <span className="flex items-center gap-1">📝 {paramCount} param{paramCount !== 1 ? 's' : ''}</span>}
                            </div>
